...
PCM ** QPulseAudioThread::s_pcm = 0;

QPulseAudioThread::QPulseAudioThread ( int _argc, char **_argv, PCM * pcm) : QThread ( ), argc ( _argc ), argv ( _argv ),  m_pcm (pcm)
{
	s_pcm = new PCM*;
	*s_pcm = m_pcm;
}
...
QPulseAudioThread::SourceContainer::const_iterator QPulseAudioThread::readSettings()
{
	...
	QSettings settings ( "prismatic.ini", QSettings::NativeFormat );
...
;
	...
	(*s_pcm)->addPCMfloat( (float*)data, length / ( sizeof ( float ) ) );
...
void QPulseAudioThread::stdout_callback ( pa_mainloop_api*a, pa_io_event *e, int fd, pa_io_event_flags_t f, void *userdata )
{
	...
	else
	{
		...
		//s_audioMutex->lock();
		PCM ** pcmPtr = static_cast<PCM**> ( userdata );
		PCM * pcm = * pcmPtr;
		...
		pcm->addPCMfloat(buffer+buffer_index, buffer_length / ( sizeof ( float )));
		//s_audioMutex->unlock();
